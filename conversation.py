...
class Conversation:
    ...
    async def start(self):
        intro = ['Good day', 'How are you doing', 'Feeling fortunate today', 'Have you ever considered your dreams']
        cancelled = [
            "Alright, maybe some other time.",
            "That's perfectly okay. My services are only for those who seek them, and I respect your decision not to have a reading today.",
        ...
        if any(word in self.init_message.content for word in ['card', 'cards', 'read', 'reading']):
            await self.respond(f'{random.choice(intro)} {self.user.mention}, I will gladly read your cards!')
            await sleep(1)
        else:
            confirm = Confirm()
            await self.respond(f'{random.choice(intro)} {self.user.mention}, would you like me to read your cards?', view=confirm)
            await confirm.wait()
            if confirm.confirmed:
                await self.draw3()
            else:
                await self.respond(random.choice(cancelled))
    ...
    async def draw3(self):
        ...
        card_number = ["Yes I see, your past card", 'let met say, your present card', 'certainly interesting! Your future card']
        with BytesIO() as buf:
            image.save(buf, "PNG")
            buf.seek(0)
        ...
        for index, card in enumerate(cards):
            await self.respond(f' {card_number[index]} is {card.name.replace("_"," ")}, {random.choice(draw_middle)} {" & ".join([", ".join(card.desc[:-1]),card.desc[-1]])} ')
